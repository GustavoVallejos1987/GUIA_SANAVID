...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function cargarTipos() {
    try {
      const res  = await fetch('http://localhost:5000/api/tipo_institucion');
  ...
  async function cargarLocalidades(depId) {
    ...
    if (!depId) return;
    try {
      const res  = await fetch(`http://localhost:5000/api/localidades/${depId}`);
      const data = await res.json(); // [{id, nombre,…},…]
      data
        .sort((a,b) => a.nombre.localeCompare(b.nombre))
        .forEach(loc => {
  ...
  async function cargarPrestadores() {
    try {
      const res   = await fetch('http://localhost:5000/api/prestadores');
